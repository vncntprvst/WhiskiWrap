...
WhiskiWrap ships with two convenience scripts located in the project root:
...
Run tracing on a short test video:

```bash
python trace_whiskers.py -v test_videos/test_video_10s.mp4
```

Execute the complete pipeline with GNN-based ID assignment:

```bash
python whisker_tracking_pipeline.py test_videos/test_video_10s.mp4 --use-gnn -p 40
```

Both scripts create a `whisker_tracking_<video>` directory next to the input
video containing the resulting Parquet or HDF5 files.
